...
def merge_with_background_pdf(generated_pdf_path):
    try:
        if background_file and os.path.exists(background_file):
            ...
            generated_pdf = PyPDF2.PdfReader(generated_pdf_path)
            writer = PyPDF2.PdfWriter()
            if len(background_pdf.pages) == 1:
                for page_num in range(len(generated_pdf.pages)):
                    bg_page = copy.deepcopy(background_pdf.pages[0])
                    bg_page.merge_page(generated_pdf.pages[page_num])
                    writer.add_page(bg_page)
            else:
                for page_num in range(len(generated_pdf.pages)):
                    if page_num < len(background_pdf.pages):
                        bg_page = copy.deepcopy(background_pdf.pages[page_num])
                        bg_page.merge_page(generated_pdf.pages[page_num])
                        writer.add_page(bg_page)
                    else:
                        writer.add_page(generated_pdf.pages[page_num])
            with open(generated_pdf_path, 'wb') as f_out:
                writer.write(f_out)
...
ACTIVATION_FILE = os.path.join(ACTIVATION_DIR, 'activationSAJA.json')
...
def check_trial_period():
    if os.path.exists(ACTIVATION_FILE):
        try:
            with open(ACTIVATION_FILE, "r", encoding="utf-8") as f:
                data = json.load(f)
            if data.get("plan") not in ["essai_7jours"]:
                return True
        except Exception:
            pass
    ...
    if platform.system() == "Windows":
        ...
        file_path = os.path.join(hidden_folder, 'windowsSaja')
    else:
...
def purchase_plan(plan):
    if plan not in ["1 mois", "1 an"]:
        return "Plan non valide", 400
    amount = "10.00" if plan == "1 mois" else "20.00"
    try:
            ...
            <form method="POST" action="{{ url_for('activation') }}">
              ...
              <div class="mb-3">
                ...
                <div class="d-flex flex-column flex-sm-row gap-2">
                  <button type="submit" name="choix" value="1 mois" class="btn btn-success flex-fill">Activer 1 mois (15.00 USD)</button>
                  <button type="submit" name="choix" value="1 an" class="btn btn-success flex-fill">Activer 1 an (25.00 USD)</button>
                </div>
              </div>
    ...
    if request.method == "POST":
        ...
        if choix == "essai":
            ...
            if activation_code:
                ...
                try:
                    amount = "10.00" if choix == "1 mois" else "20.00"
                    order_id, approval_url = create_paypal_order(amount, "USD")
...
<nav class="navbar navbar-fixed-topd-lg navbar-dark bg-dark fixed-top">
  <div class="container-fluid">
   <a class="navbar-brand" href="#">MedicSastouka</a>
...
<div class="container my-4">
  <div class="card shadow-lg">
    <div class="card-header bg-gradient-to-r from-indigo-600 to-blue-500 text-white text-center">
      <h1>{{ config.nom_clinique or config.cabinet or 'MedicSastouka' }}</h1>
      <h2>{{ config.doctor_name or 'Nom du Médecin' }} - {{ config.location or 'Lieu' }}</h2>
    ...
    <div class="card-body">
      <form method="POST" enctype="multipart/form-data" id="mainForm">
        ...
        <div class="d-flex flex-wrap justify-content-around gap-2 mt-4">
          ...
          <button type="button" class="btn btn-warning" data-bs-toggle="modal" data-bs-target="#importExcelModal">Importer Listes Prédefinies</button>
          <button type="button" class="btn btn-danger" data-bs-toggle="modal" data-bs-target="#importBackgroundModal">Importer Arrière‑plan</button>
        </div>
      </form>
    </div>
